...
# Create a combined feature for better accuracy
...
def recommend(movie_title, selected_genre):
    ...
    return recs[recs['genres'] == selected_genre]


# ---------------- UI ----------------
st.markdown("### 🎭 Select Genre")
genres = sorted(movies['genres'].unique())
...
if st.button("🚀 Recommend Movies"):
    with st.spinner("Finding the best movies for you..."):
        recs = recommend(movie_name, selected_genre)

    if len(recs) == 0:
        st.error("No similar movies found in this genre 😕")
    else:
        st.success(f"🍿 Recommendations in **{selected_genre}** genre:")
        cols = st.columns(3)
